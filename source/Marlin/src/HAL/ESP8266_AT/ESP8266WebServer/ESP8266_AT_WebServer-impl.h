...
ESP8266_AT_WebServer::ESP8266_AT_WebServer()  //delete port by zyf
  : _server(80)         //80=port
...
void ESP8266_AT_WebServer::begin(int port) //add port by zyf 
{
  _currentStatus = HC_NONE;
  _server.begin(port); //add port by zyf 
...
void ESP8266_AT_WebServer::send(int code, const char* content_type, const String& content) 
{
  ...
  if (content.length())
  {
    ...
    //TLDEBUG_LNPAIR("send:", content.c_str());
    //TLDEBUG_LNPAIR("Length:", content.length());

    //by zyf
    _currentClient.write((const uint8_t *)content.c_str(), content.length());
...
void ESP8266_AT_WebServer::send(int code, char* content_type, const String& content, size_t contentLength)
{
  ...
  memcpy((void*)type, content_type, sizeof(type));
...
void ESP8266_AT_WebServer::sendContent(const String& content, size_t size)
{
  ...
  //TLDEBUG_LNPAIR("sendContent:", content.c_str());
  //TLDEBUG_LNPAIR("Length:", size);
...
void ESP8266_AT_WebServer::send_P(int code, PGM_P content_type, PGM_P content) 
{
  ...
  memcpy_P((void*)type, (PGM_VOID_P)content_type, sizeof(type));
  _prepareHeader(header, code, (const char* )type, contentLength);
