...
void Temperature::manage_heater() {
  ...
  updateTemperaturesFromRawValues(); // also resets the watchdog
      ...
      if (degHotend(0) > _MIN(tl_E_MAX_TEMP, TEMP_SENSOR_0_MAX_TC_TMAX - 1.0)) max_temp_error(H_E0);
      if (degHotend(0) < _MAX(tl_E_MAX_TEMP, TEMP_SENSOR_0_MAX_TC_TMIN + .01)) min_temp_error(H_E0);
    #endif
...
void Temperature::init() {
  ...
  // Wait for temperature measurement to settle
    ...
    #define _TEMP_MAX_E(NR) do{ \
      const celsius_t tmax = _MIN(tl_E_MAX_TEMP, TERN(TEMP_SENSOR_##NR##_IS_CUSTOM, 2000, (int)pgm_read_word(&TEMPTABLE_##NR [TEMP_SENSOR_##NR##_MAXTEMP_IND].celsius) - 1)); \
...
void Temperature::isr() {
  ...
  //if ((do_buttons ^= true)) ui.update_buttons();  //by zyf
  ...
  void Temperature::print_heater_states(const uint8_t target_extruder
    ...
    void Temperature::wait_for_bed_heating() {
      if (isHeatingBed()) {
        ...
        //LCD_MESSAGEPGM(MSG_BED_HEATING);    //by zyf
        wait_for_bed();
        //ui.reset_status();    //by zyf
      }
    }
