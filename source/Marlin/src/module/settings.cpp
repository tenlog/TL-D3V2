...
*/
  ...
  bool MarlinSettings::save() {
    ...
    //
    #if HAS_MULTI_LANGUAGE
      //EEPROM_WRITE(ui.language);    //by zyf
      EEPROM_WRITE(1);    //by zyf
    #endif

    //TENLOG EEPROM settings saves here
    #if ENABLED(TENLOG_TOUCH_LCD)
  ...
  bool MarlinSettings::_load() {
    ...
    if (strncmp(version, stored_ver, 3) != 0) {
      ...
      //
      {
        ...
        #elif ENABLED(POWER_LOSS_RECOVERY_TL)
          const bool &recovery_enabled = plr_enabled;
        #else
      ...
      {        
        ...
        //ui.set_language(ui_language); // by zyf        
      }
      #endif

      //Tenlog eeprom setting load here.
      #if ENABLED(TENLOG_TOUCH_LCD)

        uint8_t ui_tllanguage;
        EEPROM_READ(ui_tllanguage);
        if(ui_tllanguage < 0) ui_tllanguage = 0;
        if(ui_tllanguage > 6) ui_tllanguage = 0;
        tl_languageID = ui_tllanguage;

        uint8_t ui_tlSleep;
        EEPROM_READ(ui_tlSleep);
        if(ui_tlSleep < 0) ui_tlSleep = 0;
        if(ui_tlSleep > 240) ui_tlSleep = 0;
        tl_Sleep = ui_tlSleep;

        uint8_t ui_tlECO;
        EEPROM_READ(ui_tlECO);
...
void MarlinSettings::reset() {
  ...
  void MarlinSettings::report(const bool forReplay) {
    ...
    #elif ENABLED(POWER_LOSS_RECOVERY_TL)
      CONFIG_ECHO_HEADING("Power-Loss Recovery:");
      CONFIG_ECHO_MSG("  M413 S", plr_enabled);
    #endif
     ...
     */
      ...
      //SERIAL_ECHO_MSG("  M414 S", ui.language);   //by zyf
    #endif
  ...
  */
    ...
    void MarlinSettings::plr_fn_save(int fileIndex){
      PLR_EEPROM_START(PLR_FN_EEPROM_OFFSET);
      if(fileIndex == -1){
        for(int i=0; i<13; i++){
          PLR_EEPROM_WRITE('\0');
        }
        ...
        PLR_EEPROM_WRITE(file_name_list[fileIndex]);
        PLR_EEPROM_WRITE(long_file_name_list[fileIndex]);
      }
    }
    ...
    void MarlinSettings::plr_pre_save(uint32_t lFPos, int16_t iBPos, int16_t iTPos0, int16_t iTPos1, int16_t i_dual_x_carriage_mode, float f_duplicate_extruder_x_offset, int16_t i_feedrate){
      ...
      if(lFPos > 2048 && !PRE_Write_PLR_Done){
        ...
        PRE_Write_PLR_Done = true;
      }else if(lFPos < 2048 && PRE_Write_PLR_Done){
        PRE_Write_PLR_Done = false;
      }
    }

    uint32_t MarlinSettings::plr_is_pl(){
      PLR_EEPROM_START(PLR_EEPROM_OFFSET);
      uint32_t lFPos;
      PLR_EEPROM_READ(lFPos);
      PLR_EEPROM_START(PLR_FN_EEPROM_OFFSET);
      NULLZERO(file_name_list[6]);
      NULLZERO(long_file_name_list[6]);
      PLR_EEPROM_READ(file_name_list[6]);
      PLR_EEPROM_READ(long_file_name_list[6]);

      if(lFPos > 0 && strlen(file_name_list[6]) > 3){
        return lFPos;
      }
      return 0;
    }
    ...
    void MarlinSettings::plr_recovery(){
      ...
      float f_duplicate_extruder_x_offset = 0.0;
      uint16_t i_feedrate;
