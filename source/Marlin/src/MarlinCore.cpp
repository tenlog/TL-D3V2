...
void startOrResumeJob() {
  ...
  inline void finishSDPrinting() {
    if (queue.enqueue_one_P(PSTR("M1001"))) {
      ...
      TERN_(TENLOG_TOUCH_LCD, TLSDPrintFinished());
    }
  }
...
void idle(TERN_(ADVANCED_PAUSE_FEATURE, bool no_stepper_sleep/*=false*/)) {
  ...
  // Handle Power-Loss Recovery
    ...
    TERN_(POWER_LOSS_RECOVERY, (if (printJobOngoing()) recovery.outage()));
    TERN_(POWER_LOSS_RECOVERY_TL, plr_outage());
  #endif  
...
void setup() {
  ...
  TERN_(TENLOG_TOUCH_LCD, TlLoadingMessage("Init Screen..."));
  ...
  TERN_(TENLOG_TOUCH_LCD, TlLoadingMessage("Init Hardware..."));
