 ...
 */
void GcodeSuite::G28() {  
  float original_fr = feedrate_mm_s; //remember it.. by zyf
  float original_z = current_position.z; //remember it.. by zyf
  ...
  endstops.enable(true); // Enable endstops for next homing move
    ...
    if (doX || (doY && ENABLED(CODEPENDENT_XY_HOMING) && DISABLED(HOME_Y_BEFORE_X))) {
        ...
        //idex_set_parked();  //canceled by zyf
  ...
  TERN_(FULL_REPORT_TO_HOST_FEATURE, set_and_report_grblstate(M_IDLE));
    ...
    if(!b_temp_PrintFromZHeightFound && print_from_z_target > 0.0){
      #if ENABLED(DUAL_X_CARRIAGE)
    ...
    //by zyf
    if (z_homing_height && (doX || doY) && original_z<current_position.z ) {
