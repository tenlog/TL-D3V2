...
void GcodeSuite::M413() {
  ...
  if (parser.seen('S')){
    ...
    plr_enabled = parser.value_bool();
    #endif
  }else {
    SERIAL_ECHO_START();
    SERIAL_ECHOPGM("Power-loss recovery ");
    serialprintln_onoff(TERN(POWER_LOSS_RECOVERY_TL, plr_enabled, recovery.enabled));
  }

  #if ENABLED(DEBUG_POWER_LOSS_RECOVERY) && ENABLED(POWER_LOSS_RECOVERY)
