...
void GcodeSuite::M593_report(const bool forReplay/*=true*/) {
  report_heading_etc(forReplay, F("Input Shaping"));
    ...
    SERIAL_ECHOLNPAIR("M593 X F", stepper.get_shaping_frequency(X_AXIS),
      " D", stepper.get_shaping_damping_ratio(X_AXIS)
    );
  #endif
  #if ENABLED(INPUT_SHAPING_Y)
    TERN_(INPUT_SHAPING_X, report_echo_start(forReplay));
    SERIAL_ECHOLNPAIR("M593 Y F", stepper.get_shaping_frequency(Y_AXIS),
...
void GcodeSuite::M593() {
  ...
  if (parser.seen('F')) {
    ...
    //constexpr float min_freq = float(uint32_t(STEPPER_TIMER_RATE) / 2) / shaping_time_t(-2);  //by zyf
